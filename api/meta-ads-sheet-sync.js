...
// ------------------------------------------------------------------
// Escribe (upsert) la lista de campañas de Meta Ads del mes en la
...
//   A: ID  B: Nombre  C: Estado  D: Objetivo (automático, se re-escribe
//   siempre)  E: Gasto MTD  F: Conversiones  G: CPA  H: Squad (manual)
//
// En cada sync se recalculan A-G desde la API de Meta y se PRESERVA
// solo H (Squad) por ID de campaña — igual que Google Ads preserva
...
module.exports = async (req, res) => {
  try {
    ...
    const header = ['ID', 'Nombre', 'Estado', 'Objetivo', 'Gasto MTD', 'Conversiones', 'CPA', 'Squad'];
    const rows = campaignsData.campaigns.map(c => {
      const prevSquad = existingSquads[c.id] || '';
      return [c.id, c.name, c.status || '', objetivoLabel(c.objective), c.spend, c.conversions, c.cpa !== null ? c.cpa : '', prevSquad];
    });
